...
void SubRoundKey(unsigned char * state, unsigned char * roundKey) { //xoring 128bit block with 128 byit key
	for (int i = 0; i < 16; i++) {
		state[i] ^= roundKey[i];
	}
}

void InverseMixColumns(unsigned char * state) {//inversing the mixcolumns using the mul9,mul11,mul13,mul14 tables. just reversing mixcolumns in encryption
...
void ShiftRows(unsigned char * state) {
 ...
 //Using inverse S-box as lookup table
...
void Round(unsigned char * state, unsigned char * key) {//1 round works o n128 at a time
...
void InitialRound(unsigned char * state, unsigned char * key) {//no inverse column because its decryption . Same as round
	SubRoundKey(state, key);
	ShiftRows(state);
	SubBytes(state);
}
...
void AESDecrypt(unsigned char * encryptedMessage, unsigned char * expandedKey, unsigned char * decryptedMessage)
{
	...
	for (int i = 0; i < 16; i++) {
		decryptedMessage[i] = state[i];// copying to buffer
	}
}
...
int main() {
	...
	if (infile.is_open())
	{
		...
		cout << "Read in encrypted message from message.txt" << endl;
		infile.close();
	}
